...
app.get('/booksPages', function(req, res) {
...
app.get('/booksFiction', function(req, res) {
  Book.find({ genres: "Fiction" }).skip(2).limit(3).then(function (books) {
    res.send(books);
  });
})
...
app.get('/peopleGreyAndSkinny', function(req, res) {
  Person.find({ $and:[{weight: { $lt: 70 }} ,{ $or:[{hair: "grey"} ,{eyes: "grey"}]} ] }).then(function (people) {
    res.send(people);
  });
})
//4. Find people who have at least 1 kid with grey hair
app.get('/peopleGreyHairKid', function(req, res) {
  Person.find({ "kids.hair": "grey"}).then(function (people) {
    res.send(people);
  });
})
//5. Find all the people who have at least one overweight kid, and are overweight themselves (>100)
app.get('/peopleOverweight', function(req, res) {
  Person.find({ $and: [{ "kids.weight": { $gt: 100 }} , {weight :{ $gt: 100 }}] }).then(function (people) {
    res.send(people);
  });
})